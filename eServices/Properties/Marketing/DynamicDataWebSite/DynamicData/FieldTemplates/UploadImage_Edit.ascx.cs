...
{
    public partial class UploadImage_EditField : System.Web.DynamicData.FieldTemplateUserControl
    {
        ...
        protected void CustomValidator1_ServerValidate(object source, ServerValidateEventArgs args)
        {
            if (FileUpload1.HasFile)
            {
                ...
                {
                    args.IsValid = false;
                    //CustomValidator1.ErrorMessage = String.Format("{0} is not a valid upload image type (only {1} are supported).",
                    String.Format(Convert.ToString(GetGlobalResourceObject("DynamicData", "UploadImage_FileTypeValidator_MessageFormat")),
        ...
        protected void Page_PreRender(object sender, EventArgs e)
        {
            try
            {
                this.CustomValidator1.ValidationGroup
                    = (this.Parent as System.Web.DynamicData.DynamicControl).ValidationGroup;
            }
            catch (Exception)
            {
